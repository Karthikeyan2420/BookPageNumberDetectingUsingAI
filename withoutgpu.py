...
class PageNumberDetector:
    def __init__(self, model_path):
        self.model = YOLO(model_path)  # No explicit device configuration
        self.ocr = easyocr.Reader(['en'])  # No explicit GPU configuration
    ...
    def detect_page_number(self, image_path):
        try:
            ...
            results = self.model.predict(img, conf=0.5)  # No explicit device configuration
...
class MainWindow(QMainWindow):
    ...
    def show_results(self, results, detected_pages):
        ...
        for row, (image_path, page_number) in enumerate(results.items()):
            ...
            status_label = QLabel("✅" if re.match(r'\b\d+\b|\b[IVXLCDM]+\b', page_number) else "❌")
